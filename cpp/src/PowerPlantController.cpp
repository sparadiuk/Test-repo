...
PowerPlantController::PowerPlantController()
    : kaaClient_(kaa::Kaa::newClient()) , isShutdown_(false)
{
    reportingManager_.reset(new ReportingManager(*kaaClient_));
    ...
    std::cout << "Log: kaaClient_->setConfigurationStorage" <<  std::endl;
#endif
    kaaClient_->setConfigurationStorage(std::make_shared<kaa::FileConfigurationStorage>(POWER_PLANT_CONFIGURATION_FILE));
#if POWER_PLANT_DEBUG_LOGGING
    std::cout << "Log: kaaClient_->kaaClient_->addConfigurationListener" << std::endl;
#endif
    kaaClient_->addConfigurationListener(*this);
#if POWER_PLANT_DEBUG_LOGGING
    std::cout << "Log: kaaClient_->kaaClient_->getConfiguration" << std::endl;
#endif
    kaaClient_->start();

    configuration_ = kaaClient_->getConfiguration();


#if POWER_PLANT_DEBUG_LOGGING
    std::cout << "Log: displayConfiguration" << std::endl;
#endif
...
#if POWER_PLANT_DEBUG_LOGGING
    std::cout << "Log: kaaClient_->kaaClient_->start" << std::endl;
#endif
...
{
    while (!isShutdown_) {
        kaa::KaaRootConfiguration configuration = getConfiguration();
